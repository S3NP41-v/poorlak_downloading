...
def dummy_download(k: int, clink: str, quality: str, path: str, list_only: bool) -> str:  # I/O + returning raw link
    ...
    for chunk in range(filesize):
        ...
        print(f"\x1b[{str(k + 2)};0H\x1b[2K[Dummy] Downloading: [...]{clink.split('/')[-1]} -> {path}{filename} {' ' * (len(str(_filesize)) - len(str(_done)))} {_done}MB/{_filesize}MB ~{speed}MB/s", end='', flush=True)
        # stdout.flush()

    total = round((time() - start) / 60, 2)
    print(f"\x1b[{str(k + 2)};0H\x1b[2K[Dummy] Completed: {clink} -> {path}{filename} Total Time: {total}m {' ' * (len(str(total)) - len(str(speed)))} Speed: {speed}MB/s", end='', flush=True)
    # stdout.flush()
...
def download(k: int, clink: str, quality: str, path: str, list_only: bool) -> str:  # I/O + returning raw link
    ...
    try:  # info: skipping already fully downloaded files
        open(f"{path}/{filename}", 'x')
    except FileExistsError:  # if the file exists
        with open(f"{path}/{filename}", 'rb') as f:
            if len(f.read()) == int(requests.head(raw).headers.get("content-length")):  # and if it is fully downloaded
...
def main():
    ...
    parser.add_argument("-q", required=False, choices=["hd", "sd", "lq", "vl"],
                        help="specify the quality to download [default 'hd']", default="hd")
    parser.add_argument("-o", type=str, required=False, help="specify the path to download to")

    parser.add_argument("-t", type=int, required=False, help="amount of threads to use while downloading [default 1]",
                        default=1)

    parser.add_argument("-l", type=bool, required=False, help="just create list as well as alt-list, dont download",
                        default=False)
    ...
    with multiprocessing.Pool(args.t) as pool:
        ...
        vals = list(
            zip(range(len(clinks)), clinks, [args.q] * len(clinks), [args.o] * len(clinks), [args.l] * len(clinks)))
